...
class Slashcommands(commands.Cog):
    ...
    async def cog_load(self) -> None:
        ...
        async def translate_to_english(
            ...
            text = await translate(message.content)
            await interaction.response.send_message(content=text, allowed_mentions=discord.AllowedMentions(everyone=False))
        ...
        async def stealemojis(
            ...
            if not interaction.user.guild_permissions.create_expressions:
                return await interaction.response.send_message(
                    "You are missing the `create expressions` permissions", ephemeral=True
                )
            if not message.content:
                return await interaction.response.send_message(
                    "This message has no content", ephemeral=True

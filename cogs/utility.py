...
class Utility(commands.Cog):
    ...
    @commands.command(aliases=["memberinfo", "userinfo", "whois"])
    async def profile(
        self,
        ctx: commands.Context,
    ...
    async def translate_command(self, ctx: commands.Context, *, text: str = None):
        ...
        reply = False
        if text == "":
            return await ctx.send("Missing text to translate")
        if text is None:
        ...
        if reply:
            await ctx.message.reference.resolved.reply(
                translation, allowed_mentions=discord.AllowedMentions(everyone=False)
            )
        else:
            await ctx.send(
                translation, allowed_mentions=discord.AllowedMentions(everyone=False)

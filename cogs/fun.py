...
class BlackJack(discord.ui.View):
    ...
    def initialize_game(self, user_id: int, bet: int) -> None:
        ...
        elif self.dealer.score == 21 and self.player.score != 21:  # dealer blackjack
            self.status = BlackJackStatus.LOSE
            self.embed.description = "Dealer blackjack"
        elif self.dealer.score != 21 and self.player.score == 21:  # player blackjack
...
class Fun(commands.Cog):
    ...
    async def blackjack(self, ctx: commands.Context, bet: int = 100):
        ...
        if bet < 0:
            return await ctx.send("You can't bet a negative amount", ephemeral=True)
    ...
    @commands.is_owner()
    async def _admin(self, ctx:commands.Context, user:discord.User, birthdate:typing.Annotated[datetime.date, utils.DateConverter]):
